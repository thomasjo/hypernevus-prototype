...
train_data_file = open("/root/python/shuttle.scale",'r')
train_data_strings = train_data_file.readlines(300000000000)
# train_data_strings = train_data_file.readlines(4350)
train_data_file.close()

train_data_strings = train_data_strings[:5000]
...
test_data_file = open("/root/python/shuttle.scale.t",'r')
test_data_strings = test_data_file.readlines(300000000000)
# test_data_strings = test_data_file.readlines(1450)
test_data_file.close()

test_data_strings = test_data_strings[:1000]
...
# affinity_matrix = np.arange(5) * np.arange(5)[:, np.newaxis]
affinity_matrix = affinity_matrix.astype(np.float64)
...
# import math
# # print(np.exp(-5 * math.pow(distance.euclidean(train_data[0], train_data[1]), 2)))
# # print(np.exp(-5 * math.pow(distance.euclidean(train_data[0], train_data[2]), 2)))

laplacian, dd = csgraph.laplacian(affinity_matrix, normed=True, return_diag=True)
# # laplacian, dd = csgraph.laplacian(affinity_matrix, normed=False, return_diag=True)
# # laplacian *= -1.
...
def normed_laplacian_cell(matrix, i, j):
    deg_i = np.sum(matrix[i])

    if i == j and deg_i != 0:
        return 1.0
    elif matrix[i, j] != 0:
        deg_j = np.sum(matrix[j])
        return -matrix[i, j] / np.sqrt(deg_i * deg_j)

    return -0.0

# M = np.array(affinity_matrix)
# np.fill_diagonal(M, 0)
# m, n = laplacian.shape
# L = np.empty((m, n))
# for i in range(m):
#     for j in range(n):
#         L[i, j] = normed_laplacian_cell(M, i, j)
#         # L[i, j] = laplacian_cell(M, i, j)
...
# print(np.matmul(np.matmul(D_sqrt_inv, L), D_sqrt_inv))
...
records_file = open('/root/output/sgd_cost_nmi_2_50_60k_ada_warmstart.csv','w')
for i in range(ncol):
    tmpstr = ''
    for j in range(nrow):
